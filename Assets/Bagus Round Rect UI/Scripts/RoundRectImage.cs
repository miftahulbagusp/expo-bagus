...
/* Atlas Round Rect UI
 * Author: Miftahul Bagus P.
 * miftahulbagus@gmail.com
 */

namespace UnityEngine.UI.ProceduralImage {

	[ExecuteInEditMode]
	[AddComponentMenu("UI/Atlas Round Rect Image")]
	public class RoundRectImage : Image {
		...
		private RoundRectImageModifier modifier;
		...
		protected RoundRectImageModifier Modifier {
			get {
				if (modifier == null) {
					//try to get the modifier on the object.
					modifier = this.GetComponent<RoundRectImageModifier>();
					//if we did not find any modifier
					if(modifier == null){
						//Add free modifier
						ModifierType = typeof(FreeRoundRectModifier);
					}
				}
				return modifier;
			}
		...
		public System.Type ModifierType {
			...
			set {
				if(this.GetComponent<RoundRectImageModifier>()!=null){
					Destroy(this.GetComponent<RoundRectImageModifier>());
				}
		...
		void Init (){
			...
			if (materialInstance == null) {
				materialInstance = new Material (Shader.Find("Atlas/UI/Round Rect UI"));
			}
